...
export default function TrackingPage({ params }: { params: { envelopeId: string } }) {
  ...
  useEffect(() => {
    ...
    const load = async (initial = false) => {
      ...
      setError("");
      console.info("[tracking.detail] envelopeId", envelopeId);
      try {
        ...
        if (cancelled) return;
        console.info("[tracking.detail] envelope response", full.status);
        ...
        console.error("[tracking.detail] load failed", msg);
        setError(msg);
      } finally {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-300">
              Versions: {documentMeta?.versions?.length || 0}
            </p>
      ...
      <section className="glass rounded-xl border border-white/70 p-5">
        ...
        {envelope.recipients.length === 0 ? <p className="mt-3 text-sm text-slate-500">No recipients found.</p> : null}
      </section>
    </div>
  );
}
